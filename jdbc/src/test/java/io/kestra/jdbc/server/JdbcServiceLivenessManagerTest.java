...
class JdbcServiceLivenessManagerTest {
    ...
    @Mock
    public ServiceLivenessUpdater executor;
    ...
    void shouldRunOnStateTransitionFailureWhenTimeoutForWorker() {
        ...
        Mockito.when(executor.update(any(ServiceInstance.class), any(Service.ServiceState.class))).thenReturn(response);
        serviceLivenessManager.run(now); // SUCCEED

        // Simulate exception on each transition
        Mockito.when(executor.update(any(ServiceInstance.class), any(Service.ServiceState.class))).thenThrow(new RuntimeException());
